...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($usuario) || empty($senha)) {
        ...
        try {
            ...
            if ($existe > 0) {
                $mensagem = "O operador '$usuario' já está cadastrado.";
            } else {
                $stmt = $pdo->prepare("
                    INSERT INTO usuarios (usuario, senha, tipo)
                    VALUES (AES_ENCRYPT(?, ?), AES_ENCRYPT(?, ?), 'operador')
                ");
                $stmt->execute([$usuario, $chave_secreta, $senha, $chave_secreta]);
                $mensagem = "Operador '$usuario' cadastrado com sucesso!";
            }
        } catch (PDOException $e) {
            $mensagem = "Erro ao cadastrar operador: " . htmlspecialchars($e->getMessage());
        }
    }
}
...
<body>
    ...
    <ul>
        <?php foreach ($operadores as $operador): ?>
            <li><?php echo htmlspecialchars($operador['usuario_dec']); ?></li>
        <?php endforeach; ?>
    </ul>
